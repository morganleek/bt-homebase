<?php
  ...
  function hb_search() {
    ...
    if( !$results ) {
      ...
      // Categories via DB
      // $sql = 'SELECT *,
      //       IF(`name`         REGEXP "%1$s", 20, 0)
      //     + IF(`description`  REGEXP "%1$s", 5,  0)
      //     + IF(`slug`         REGEXP "%1$s", 1,  0)
      //   AS `weight`
      //   FROM (
      //     SELECT T.`name`, T.`slug`, TT.* FROM `wp_term_taxonomy` AS TT 
      ...
      // // $results['cat_sql'] = $prepare;
      // $sql_results = $wpdb->get_results( $prepare );
      
      // if($sql_results) {
      //   foreach($sql_results as $sql_result) {
      //     // $cat_thumb_id = get_woocommerce_term_meta( $sql_result->term_id, 'thumbnail_id', true );
      //     $cat_thumb_id = get_term_meta( $sql_result->term_id, 'thumbnail_id', true );
      //     $image = wp_get_attachment_url( $cat_thumb_id );
  
      //     $results['categories'][] = [ 
			// 			'url'         => get_term_link( $sql_result ),
			// 			'title'       => $sql_result->name,
			// 			'description' => wp_trim_words( $sql_result->description, 8, '&hellip;' ),
			// 			'price'       => '',
			// 			'thumbnail'   => $image,
			// 		];
      //   }
